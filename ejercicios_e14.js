...
//Crea una función flecha que reciba el arreglo y devuelva un nuevo arreglo
//solo con los nombres de todos los productos.
const obtenerNombreProductos = (arr) => arr.map(prod => prod.nombre);
console.log(obtenerNombreProductos(productos));

//2. Crea una función flecha que reciba el arreglo y retorne todos los productos
//cuya categoría sea "Muebles".
const obtenerMuebles = (arr) => arr.filter(prod => prod.categoria === "Muebles");
console.log(obtenerMuebles(productos));

//3. Crea una función flecha que reciba el arreglo y devuelva el primer producto
//cuyo precio sea mayor a 1000.
const obtenerProductoCaro = (arr) => arr.find(prod => prod.precio > 1000);
console.log(obtenerProductoCaro(productos));
...
const calcularInventarioTotal = (arr) => arr.reduce((total, prod) => total + (prod.precio * prod.stock), 0);
console.log(calcularInventarioTotal(productos));

//5. Crea una función flecha que reciba el arreglo y determine si todos los
//productos tienen un stock mayor a 0.
const todosConsStock = (arr) => arr.filter(prod => prod.stock > 0).length === arr.length;
console.log(todosConsStock(productos)); 

//Crea una función flecha que reciba el arreglo y verifique si al menos un
//producto pertenece a la categoría "Electrodomésticos".
const tieneElectrodomesticos = (arr) => arr.some(prod => prod.categoria === "Electrodomésticos");
console.log(tieneElectrodomesticos(productos));
...
const ordenarPorPrecio = (arr) => [...arr].sort((a, b) => a.precio - b.precio);
...
const ordenarPorNombreInverso = (arr) => [...arr].sort((a, b) => b.nombre.localeCompare(a.nombre));
...
const ProductoBajoStock = (arr) => arr.filter(prod => prod.stock < 10);
console.log(ProductoBajoStock(productos));

//10.Crea una función flecha que reciba el arreglo y obtenga el promedio de los
//precios de todos los productos.
const promedioPrecios = (arr) => arr.reduce ((acc, prod, _, array) => acc + prod.precio / array.length, 0);
